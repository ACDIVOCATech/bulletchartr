...
IndicatorData <- readxl::read_xlsx("data/Indicators_Targets_full.xlsx", sheet = "Sheet1")
...
```{r, echo=FALSE}
IndicatorData %>% select(indicator_name, actual, actual_lastweek, actual_lastyear, target) %>% glimpse()
```
...
The vertical line showing `Today` can be customized depending on whehter you are measuring by a fiscal year, a calendar year, or a custom date. `Today` shown on a Fiscal Year calendar (considering that today's physical date is `r Sys.Date()`.) is show below:
